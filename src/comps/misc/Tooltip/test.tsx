...
describe("Comps_misc\Tooltip", () => {
  ...
  // it("should render a '' on the Comps_misc\Tooltip and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_misc\Tooltip.rename(oneProduct);
  //   renderWithRematchStore(<Comps_misc\Tooltip />, store);
  ...
  // it("should reset the Comps_misc\Tooltip to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_misc\Tooltip.rename(otherProduct);
  //   renderWithRematchStore(<Comps_misc\Tooltip />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
