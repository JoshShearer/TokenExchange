...
describe("Comps_layout_MTBApp", () => {
  ...
  // it("should render a '' on the Comps_layout_MTBApp and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_MTBApp.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_MTBApp />, store);
  ...
  // it("should reset the Comps_layout_MTBApp to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_MTBApp.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_MTBApp />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
