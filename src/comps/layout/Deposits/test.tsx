...
describe("Comps_layout_Deposits", () => {
  ...
  // it("should render a '' on the Comps_layout_Deposits and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_Deposits.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_Deposits />, store);
  ...
  // it("should reset the Comps_layout_Deposits to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_Deposits.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_Deposits />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
