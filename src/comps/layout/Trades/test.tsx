...
describe("Comps_layout_Trades", () => {
  ...
  // it("should render a '' on the Comps_layout_Trades and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_Trades.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_Trades />, store);
  ...
  // it("should reset the Comps_layout_Trades to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_Trades.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_Trades />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
