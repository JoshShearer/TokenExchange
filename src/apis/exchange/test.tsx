...
describe("apis_exchange", () => {
  ...
  // it("should render a '' on the apis_exchange and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.apis_exchange.rename(oneProduct);
  //   renderWithRematchStore(<apis_exchange />, store);
  ...
  // it("should reset the apis_exchange to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.apis_exchange.rename(otherProduct);
  //   renderWithRematchStore(<apis_exchange />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
